...
#[
  ...
  / 方向兴趣: 数据基础设施、元数据管理、分布式系统、AI Infrastructure
  / 课程与自学: 数据结构、计算机组成原理；持续学习数据库系统、操作系统、计算机网络
]
...
#text(
  size: 10pt,
)[*面向学生创新组织的团队在线协作文档系统（类飞书文档，开发中），围绕组织知识沉淀与协作数据管理，目标支持多人实时编辑、版本追踪、权限控制与后续检索扩展。*]
- 独立负责需求梳理、系统设计、原型实现与后端开发，从协作文档场景抽象文档数据、版本元信息与权限边界。
- 后端使用 Go 设计文档服务与 API 层，围绕数据写入、版本追踪、权限校验拆分核心模块，强调可维护性与后续扩展。
- 基于 PostgreSQL 设计文档、版本、权限与元信息模型，完成基础查询与 Schema 设计，支持历史回溯、目录组织与面向后续检索能力的扩展。
...
- 结合 FPGWiki 的建设进展，持续梳理文档结构、目录组织与权限边界，推进原型迭代与知识沉淀流程。
- 协助整理组织技术文档与基础设施相关资料，关注文档质量、信息可追踪性与成员协作效率。
...
#[
  #grid(
    ...
    skills-box(
      ...
      side: "left",
      points: (
        "熟悉 Linux / Shell 开发环境，能够使用命令行工具与脚本完成开发、排障与自动化处理。",
        "主攻 Go，具备 TypeScript 开发经验，能够完成后端接口开发、基础 Web 原型实现与前后端联调。",
        "具备 SQL 与 PostgreSQL 基础，能够进行关系模型设计、查询编写与文档/权限相关数据建模。",
      ),
    ),
    skills-box(
      title: "工程实践与方向",
      side: "right",
      points: (
        "掌握 Git 基本协作流程，关注服务模块化、版本管理与可维护性。",
        "关注数据基础设施、元数据管理、权限治理与协作数据一致性等问题，持续学习相关系统设计。",
        "持续维护个人 GitHub 与技术博客，记录项目实现、数据库设计与工具链实践。",
      ),
    ),
  )
]
